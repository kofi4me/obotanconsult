...
try{
 ...
 expect((await post(payload(slots[0].start))).r.status,400,'CV required');
 expect((await post(cv(payload(slots[0].start),'not a PDF'))).r.status,400,'invalid PDF rejected');
 expect((await post(cv(payload(slots[0].start+60000)))).r.status,400,'off-grid appointment rejected');
 const form=cv(payload(slots[0].start));const first=await post(form);expect(first.r.status,201,'booking and upload saved');
 ...
 expect((await post(cv(payload(slots[0].start)))).r.status,409,'duplicate slot rejected');
 const raced=await Promise.all([post(cv(payload(slots[1].start))),post(cv(payload(slots[1].start)))]);expect(raced.filter(x=>x.r.status===201).length,1,'only one concurrent reservation');expect(raced.filter(x=>x.r.status===409).length,1,'concurrent loser gets conflict');
 ...
 const document=await request(`/api/admin/submissions/${row.id}/document`,{headers:admin});expect(document.status,200,'admin downloads CV');assert(document.headers.get('content-disposition').startsWith('attachment'));assert((await document.text()).startsWith('%PDF-'));
 ...
 console.log(`PASS: ${assertions} booking, upload, authorization, conflict and cancellation assertions.`);
}finally{for(const id of made){const r=await request(`/api/admin/submissions/${id}`,{method:'DELETE',headers:{...admin,Origin:base}});assert.equal(r.status,200,'test record cleanup')}console.log('Local test records and CVs removed.');}
