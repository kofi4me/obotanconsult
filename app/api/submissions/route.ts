import {database,bucket,json,sameOrigin,limitedBody,rateLimit} from "@/lib/server";
...
export async function POST(request:Request){
 ...
 try{
  ...
  const file=form.get("cv");let fileName:string|null=null,fileSize:number|null=null;
  if(file instanceof File&&file.size>0){
   if(file.size>5*1024*1024||!file.name.toLowerCase().endsWith(".pdf"))return json({error:"Please upload a PDF of 5 MB or less."},400);
   const content=await file.arrayBuffer();if(new TextDecoder().decode(content.slice(0,5))!=="%PDF-")return json({error:"The document must be a valid PDF file."},400);
   fileName=file.name.replace(/[\r\n\x00-\x1f]/g,"").slice(0,180);fileSize=file.size;uploaded=`cv/${crypto.randomUUID()}.pdf`;
   await bucket().put(uploaded,content,{httpMetadata:{contentType:"application/pdf"}});
  }else if(service.slug!=="business-docs")return json({error:"Please attach your CV as a PDF."},400);
  await database().prepare("INSERT INTO submissions (id,service,name,email,phone,field,notes,evidence,file_key,file_name,file_size,slot,reserved_slot,status,admin_notes,consent_version,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,'new','',?,?,?)").bind(id,service.slug,name,email,phone,field,notes,JSON.stringify(evidence),uploaded||null,fileName,fileSize,slot,slot,DISCLOSURE_VERSION,Date.now(),Date.now()).run();
  return json({reference:id,appointment:slotInfo(slot)},201);
 }catch(error){
  if(uploaded)try{await bucket().delete(uploaded)}catch{console.error("orphan_upload_cleanup_needed")}
  ...
  if(message==="BODY_TOO_LARGE")return json({error:"Please upload a PDF no larger than 5 MB."},413);
